...
export function search(haystack: FollowsData[], needle: string): any[] {
    const maxDistance = 1;
    const res = haystack.map((broadcaster) => {
        const distance = getMinDistance(needle.toLowerCase(), broadcaster.broadcaster_name.toLowerCase());
        const sim = 1 - normalized(distance, needle.length, broadcaster.broadcaster_name.length);
